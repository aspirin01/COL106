...
public class Heap{
	...
	protected int[] values_array,keys_array;                    // Stores the address of node corresponding to the keys
	...
	*/
	...
	public ArrayList<Integer> getMax() throws Exception{
		...
		*/
		...
		if(this.values_array.length==0){
			System.out.println(NullRootException);
			if(this.keys_array.length==0){
		...
		maxHelper(this.root,max_keys);
		return max_keys;
	}

	void maxHelper(Node head, ArrayList<Integer> arr){
	...
	private void insertHelper(Node head, int key, int value){
		if(this.root==null){
			this.root.key=key;
		...
		Node parent = nodes_array[keys_array[index]];


		Node temp = new Node(key,value,parent);
